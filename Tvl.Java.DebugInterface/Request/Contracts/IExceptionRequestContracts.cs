namespace Tvl.Java.DebugInterface.Request.Contracts
{
    ...
    [ContractClassFor(typeof(IExceptionRequest))]
    internal abstract class IExceptionRequestContracts : IExceptionRequest
    {
        #region IExceptionRequest Members

        public IReferenceType Exception
        {
            get
            {
                Contract.Ensures(Contract.Result<IReferenceType>() == null || this.GetVirtualMachine().Equals(Contract.Result<IReferenceType>().GetVirtualMachine()));
